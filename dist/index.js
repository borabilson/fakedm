var __plugin = (() => {
  ...
  var vendetta = g.vendetta || {};
  var metro = vendetta.metro || {};
  var common = metro.common || {};
  var ui = vendetta.ui || {};
  var findByProps = metro.findByProps || (() => null);
  var findByStoreLazy = metro.findByStoreLazy || (() => null);
  var FluxDispatcher = common.FluxDispatcher || findByProps("dispatch", "subscribe") || { dispatch: () => {
  } };
  var showToast = ui.toasts?.showToast || ((msg) => {
  });
  var storage = vendetta.storage || {};
  var UserStore = findByStoreLazy("UserStore") || findByProps("getUser", "getCurrentUser");
  var SelectedChannelStore = findByStoreLazy("SelectedChannelStore") || findByProps("getChannelId");
  var ChannelStore = findByStoreLazy("ChannelStore") || findByProps("getChannel");
  storage.fakes ??= [];
  var _idCounter = 0;
  function uniqueSnowflake(date) {
    const offset = _idCounter++ % 4096;
    const ms = Math.max(0, date.getTime() - 14200704e5);
    const high = Math.floor(ms / 1e3);
    const low = ms % 1e3 * 4194304 + offset;
    return (BigInt(ms) * BigInt(4194304) + BigInt(offset)).toString();
  }
  ...
  function removePersisted(channelId, ids) {
    if (Array.isArray(storage.fakes)) {
  ...
  function avatarUrl(user) {
    ...
    const idx = user.discriminator && user.discriminator !== "0" ? parseInt(user.discriminator) % 5 : 0;
  ...
  function inject(channelId, author, content, date, persistedId) {
    ...
    if (!persistedId && Array.isArray(storage.fakes)) {
      storage.fakes.push({
        type: "message",
        channelId,
        authorId: author.id,
  ...
  function doRestore() {
    ...
    if (!Array.isArray(fakes) || !fakes.length) return;
